...
const CONFIG = Object.freeze({
    ...
    // true: buang QUIC Initial pada UDP/443; STUN/TURN tetap diizinkan.
...
function rejectUdpPacket(target, payload) {
    return Boolean(CONFIG.REJECT_UDP_443 && Number(target?.port) === 443 && isQuicInitial(payload));
}
...
async function serveDirectUDP(socket, reader, target) {
    ...
    try {
        ...
        for (;;) {
            ...
            if (!rejectUdpPacket(target, payload)) {
...
class MuxConnection {
    ...
    async handleKeep(frame) {
        ...
        if (!rejectUdpPacket(target, frame.data)) {
